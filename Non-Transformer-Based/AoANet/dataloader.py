...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # print(f"Requested index: {ix}, dataset images count: {len(self.info['images'])}")
        # image_id = str(self.info['images'][ix]['id'])
        # # print(f"Accessing image ID: {image_id}")

        # att_feat = self.att_loader.get(image_id)
        # if att_feat is None:
        #     raise KeyError(f"Attention features for image ID {image_id} not found")
        if self.use_att:
