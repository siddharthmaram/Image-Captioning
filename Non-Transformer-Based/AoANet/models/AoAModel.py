...
class AoA_Decoder_Core(nn.Layer):
    ...
    def forward(self, xt, mean_feats, att_feats, p_att_feats, state, att_masks=None):
        att_ctx = mean_feats + self.ctx_drop(state[0][1])
        xt = xt.squeeze(1)  # Removes dimension 1, shape becomes [3, 1024]
        # print("att_ctx shape:", att_ctx.shape)
        # print("xt shape:", xt.shape)
        att_lstm_input = paddle.concat([xt, att_ctx], axis=1)
...
class AoAModel(AttModel):
    ...
    def _prepare_feature(self, fc_feats, att_feats, att_masks):
        ...
        assert not paddle.isnan(fc_feats).any(), "NaN in fc_feats"
        assert not paddle.isnan(att_feats).any(), "NaN in att_feats"
