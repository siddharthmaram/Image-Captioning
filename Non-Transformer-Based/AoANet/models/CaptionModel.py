...
class CaptionModel(nn.Layer):
    ...
    def beam_search(self, state, *args, opt=None):
        ...
        while True:
            ...
            # Ensure shapes: seqs_prev_word_inds [num_beams, step], next_word_inds [num_beams]
            if seqs_prev_word_inds.ndim > 2:
                seqs_prev_word_inds = seqs_prev_word_inds.squeeze()  # Squeeze any singleton dimensions
            next_word_inds = next_word_inds.unsqueeze(-1)  # Shape [num_beams, 1]
            # Concatenate along last axis to grow sequence
            seqs = paddle.concat([seqs_prev_word_inds, next_word_inds], axis=-1)  # (num_beams, step+1)
